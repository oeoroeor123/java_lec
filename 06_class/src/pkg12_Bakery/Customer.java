...
public class Customer {
  ...
  public void buy(Bakery bakery, int bread, int money) {
    ...
    if(this.money < money) { // 구매할 때 낼 돈 < 내가 가진 돈
      System.out.println("내가 가진 돈이 부족하다.(" + (money-this.money) + "원 부족)");
      return;
    }
    ...
    if(breadNChange != null) { // 구매 성공 (빵집이 있거나, 빵집에서 퇴짜 맞지 않았을 경우)
      this.bread += breadNChange.getBread(); // 잔돈과 빵 중에서 빵만 빼기
      this.money -= (money - breadNChange.getChange()); // 구매할 때 낸 돈 - 잔돈 = 실제 빵 가격
    }
